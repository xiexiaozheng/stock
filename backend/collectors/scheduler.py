...
def _infer_ts_code(code: str) -> str:
    pure = normalize_stock_code(code)
...
class DataCollector:
    ...
    def _resolve_old_codes(self, stock_codes: Optional[List[str]] = None) -> List[str]:
        if stock_codes is None:
            return [
                r[0]
                for r in self.db.query(Stock.stock_code).filter(Stock.is_active == True).all()
            ]

        requested = list(dict.fromkeys(normalize_stock_code(code) for code in stock_codes))
        return [
            r[0]
            for r in (
                self.db.query(Stock.stock_code)
                .filter(Stock.is_active == True, Stock.stock_code.in_(requested))
                .all()
            )
        ]

    def _resolve_core_codes(self, stock_codes: Optional[List[str]] = None) -> List[str]:
        if stock_codes is None:
            return [
                r[0]
                for r in (
                    self.db.query(StockBasic.ts_code)
                    .filter(StockBasic.is_delist.is_(False))
                    .all()
                )
            ]

        requested = list(dict.fromkeys(_infer_ts_code(code) for code in stock_codes))
        rows = (
            self.db.query(StockBasic.ts_code)
            .filter(StockBasic.is_delist.is_(False), StockBasic.ts_code.in_(requested))
            .all()
        )
    ...
    def run_incremental_update(self, stock_codes: Optional[List[str]] = None):
        """
        增量更新流程（每日运行）：
        1. 更新股票列表（旧表 + 新核心表）
        2. 更新最新交易日行情
        ...
        # 只更新数据库中已有的股票（旧表）
        codes = self._resolve_old_codes(stock_codes)
        total = len(codes)
        if stock_codes is None:
            logger.info(f"增量更新 {total} 只股票的行情和估值数据")
        else:
            logger.info(f"指定增量更新 {total} 只股票的行情和估值数据")
        ...
        core_codes = self._resolve_core_codes(stock_codes)
    ...
    def run_full_refresh(self, stock_codes: Optional[List[str]] = None):
        ...
        if stock_codes is None:
            ...
            logger.info(f"指定全量刷新 {total} 只股票")
    ...
    def _run():
        ...
        try:
            ...
            if task_type == "full_refresh":
                ...
                collector.run_incremental_update(stock_codes)
